...
// created by wsf at 2015.10.09

/**
 * @brief Computes the My loss
 */
template <typename Dtype>
class MyLossLayer : public LossLayer<Dtype> {
 public:
  ...
  /**
   * Unlike most loss layers, in the MyLossLayer we can backpropagate
...
// created by wsf at 2016.3.01
/**
 * @brief Computes the My l2 loss
 */
template <typename Dtype>
class MyL2LossLayer : public LossLayer<Dtype> {
 public:
  explicit MyL2LossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  
  virtual inline int ExactNumBottomBlobs() const { return 3; }
  virtual inline const char* type() const { return "MyL2Loss"; }
  /**
   * Unlike most loss layers, in the EuclideanLossLayer we can backpropagate
   * to both inputs -- override to return true and always allow force_backward.
   */
