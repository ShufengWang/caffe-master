...
WeightWindowDataLayer<Dtype>::~WeightWindowDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void WeightWindowDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "weight Window data layer:" << std::endl
      << "  foreground (object) overlap threshold: "
      << this->layer_param_.weight_window_data_param().fg_threshold() << std::endl
      << "  background (non-object) overlap threshold: "
      << this->layer_param_.weight_window_data_param().bg_threshold() << std::endl
      << "  foreground sampling fraction: "
      << this->layer_param_.weight_window_data_param().fg_fraction() << std::endl
      << "  cache_images: "
      << this->layer_param_.weight_window_data_param().cache_images() << std::endl
      << "  root_folder: "
      << this->layer_param_.weight_window_data_param().root_folder();

  cache_images_ = this->layer_param_.weight_window_data_param().cache_images();
  string root_folder = this->layer_param_.weight_window_data_param().root_folder();
  ...
  std::ifstream infile(this->layer_param_.weight_window_data_param().source().c_str());
  CHECK(infile.good()) << "Failed to open window file "
      << this->layer_param_.weight_window_data_param().source() << std::endl;
  ...
  do {
    ...
    const float fg_threshold =
        this->layer_param_.weight_window_data_param().fg_threshold();
    const float bg_threshold =
        this->layer_param_.weight_window_data_param().bg_threshold();
    for (int i = 0; i < num_windows; ++i) {
      int label, x1, y1, x2, y2;
      float overlap,weight;
      infile >> label >> overlap >> x1 >> y1 >> x2 >> y2 >> weight;

      vector<float> window(WeightWindowDataLayer::NUM);
      window[WeightWindowDataLayer::IMAGE_INDEX] = image_index;
      window[WeightWindowDataLayer::LABEL] = label;
      window[WeightWindowDataLayer::OVERLAP] = overlap;
      window[WeightWindowDataLayer::X1] = x1;
      window[WeightWindowDataLayer::Y1] = y1;
      window[WeightWindowDataLayer::X2] = x2;
      window[WeightWindowDataLayer::Y2] = y2;
	  window[WeightWindowDataLayer::WEIGHT] = weight;

	  
      // add window to foreground list or background list
      if (overlap >= fg_threshold) {
        int label = window[WeightWindowDataLayer::LABEL];
        CHECK_GT(label, 0);
        ...
        window[WeightWindowDataLayer::LABEL] = 0;
        window[WeightWindowDataLayer::OVERLAP] = 0;
  ...
  LOG(INFO) << "Amount of context padding: "
      << this->layer_param_.weight_window_data_param().context_pad();

  LOG(INFO) << "Crop mode: "
      << this->layer_param_.weight_window_data_param().crop_mode();

  // image
  const int crop_size = this->transform_param_.crop_size();
  CHECK_GT(crop_size, 0);
  const int batch_size = this->layer_param_.weight_window_data_param().batch_size();
...
template <typename Dtype>
unsigned int WeightWindowDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void WeightWindowDataLayer<Dtype>::InternalThreadEntry() {
  ...
  const Dtype scale = this->layer_param_.weight_window_data_param().scale();
  const int batch_size = this->layer_param_.weight_window_data_param().batch_size();
  const int context_pad = this->layer_param_.weight_window_data_param().context_pad();
  const int crop_size = this->transform_param_.crop_size();
  const bool mirror = this->transform_param_.mirror();
  const float fg_fraction =
      this->layer_param_.weight_window_data_param().fg_fraction();
  Dtype* mean = NULL;
  int mean_off = 0;
  int mean_width = 0;
  int mean_height = 0;
  ...
  const string& crop_mode = this->layer_param_.weight_window_data_param().crop_mode();
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      pair<std::string, vector<int> > image =
          image_database_[window[WeightWindowDataLayer<Dtype>::IMAGE_INDEX]];

      cv::Mat cv_img;
      if (this->cache_images_) {
        pair<std::string, Datum> image_cached =
          image_database_cache_[window[WeightWindowDataLayer<Dtype>::IMAGE_INDEX]];
      ...
      int x1 = window[WeightWindowDataLayer<Dtype>::X1];
      int y1 = window[WeightWindowDataLayer<Dtype>::Y1];
      int x2 = window[WeightWindowDataLayer<Dtype>::X2];
      int y2 = window[WeightWindowDataLayer<Dtype>::Y2];
      ...
      // get window label
      top_label[item_id] = window[WeightWindowDataLayer<Dtype>::LABEL];
	    
	  // get window weight
      top_weight[item_id] = window[WeightWindowDataLayer<Dtype>::WEIGHT];
...
template <typename Dtype>
void WeightWindowDataLayer<Dtype>::LayerSetUp(
  ...
  // LOG(INFO) << " WEIGHT WINDOW DATA LAYER SET UP";
...
template <typename Dtype>
void WeightWindowDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
INSTANTIATE_CLASS(WeightWindowDataLayer);
REGISTER_LAYER_CLASS(WeightWindowData);
