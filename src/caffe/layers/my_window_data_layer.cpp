...
MyWindowDataLayer<Dtype>::~MyWindowDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void MyWindowDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "Window data layer:" << std::endl
      ...
      << this->layer_param_.my_window_data_param().fg_threshold() << std::endl
      << "  background (non-object) overlap threshold: "
      << this->layer_param_.my_window_data_param().bg_threshold() << std::endl
      << "  foreground sampling fraction: "
      << this->layer_param_.my_window_data_param().fg_fraction() << std::endl
      << "  cache_images: "
      << this->layer_param_.my_window_data_param().cache_images() << std::endl
      << "  root_folder: "
      << this->layer_param_.my_window_data_param().root_folder();

  cache_images_ = this->layer_param_.my_window_data_param().cache_images();
  string root_folder = this->layer_param_.my_window_data_param().root_folder();
  ...
  std::ifstream infile(this->layer_param_.my_window_data_param().source().c_str());
  CHECK(infile.good()) << "Failed to open window file "
      << this->layer_param_.my_window_data_param().source() << std::endl;

  std::ifstream spv_infile(this->layer_param_.my_window_data_param().spv_source().c_str());
  CHECK(spv_infile.good()) << "Failed to open spv file "
      << this->layer_param_.my_window_data_param().spv_source() << std::endl;
	
  const int spv_length = this->layer_param_.my_window_data_param().spv_length();
  ...
  do {
    ...
    const float fg_threshold =
        this->layer_param_.my_window_data_param().fg_threshold();
    const float bg_threshold =
        this->layer_param_.my_window_data_param().bg_threshold();
    for (int i = 0; i < num_windows; ++i) {
      int label, x1, y1, x2, y2;
      float overlap,weight;
      infile >> label >> overlap >> x1 >> y1 >> x2 >> y2 >> weight;

      vector<float> window(MyWindowDataLayer::NUM);
      window[MyWindowDataLayer::IMAGE_INDEX] = image_index;
      window[MyWindowDataLayer::LABEL] = label;
      window[MyWindowDataLayer::OVERLAP] = overlap;
      window[MyWindowDataLayer::X1] = x1;
      window[MyWindowDataLayer::Y1] = y1;
      window[MyWindowDataLayer::X2] = x2;
      window[MyWindowDataLayer::Y2] = y2;
	  window[MyWindowDataLayer::WEIGHT] = weight;

	  vector<float> spv(spv_length);
	  for (int j=0;j<spv_length;j++)
		spv_infile >> spv[j];

      // add window to foreground list or background list
      if (overlap >= fg_threshold) {
        int label = window[MyWindowDataLayer::LABEL];
        CHECK_GT(label, 0);
        fg_windows_.push_back(window);
		fg_spv_.push_back(spv);

        label_hist.insert(std::make_pair(label, 0));
        label_hist[label]++;
      } else if (overlap < bg_threshold) {
        // background window, force label and overlap to 0
        window[MyWindowDataLayer::LABEL] = 0;
        window[MyWindowDataLayer::OVERLAP] = 0;
        bg_windows_.push_back(window);
		bg_spv_.push_back(spv);        
		label_hist[0]++;
      }
    }
  ...
  LOG(INFO) << "Amount of context padding: "
      << this->layer_param_.my_window_data_param().context_pad();

  LOG(INFO) << "Crop mode: "
      << this->layer_param_.my_window_data_param().crop_mode();

  // image
  const int crop_size = this->transform_param_.crop_size();
  CHECK_GT(crop_size, 0);
  const int batch_size = this->layer_param_.my_window_data_param().batch_size();
  ...
  //this->prefetch_weight_.mutable_cpu_data();
  this->prefetch_weight_.Reshape(weight_shape);

  LOG(INFO) <<" SPV_LENGTH: " << spv_length;
  // spv
  vector<int> spv_shape(2);
  spv_shape[0] = batch_size;
  spv_shape[1] = spv_length;
  top[3]->Reshape(spv_shape);
  //this->prefetch_spv_.mutable_cpu_data();
  this->prefetch_spv_.Reshape(spv_shape);
...
template <typename Dtype>
unsigned int MyWindowDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
void MyWindowDataLayer<Dtype>::InternalThreadEntry() {
  ...
  Dtype* top_weight = this->prefetch_weight_.mutable_cpu_data();
  Dtype* top_spv = this->prefetch_spv_.mutable_cpu_data();
  
  const Dtype scale = this->layer_param_.my_window_data_param().scale();
  const int batch_size = this->layer_param_.my_window_data_param().batch_size();
  const int context_pad = this->layer_param_.my_window_data_param().context_pad();
  const int crop_size = this->transform_param_.crop_size();
  const bool mirror = this->transform_param_.mirror();
  const float fg_fraction =
      this->layer_param_.my_window_data_param().fg_fraction();
  Dtype* mean = NULL;
  int mean_off = 0;
  int mean_width = 0;
  int mean_height = 0;
  ...
  const string& crop_mode = this->layer_param_.my_window_data_param().crop_mode();
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
	  ...
	  vector<float> spv = (is_fg) ?
          fg_spv_[rand_index % fg_spv_.size()] :
          bg_spv_[rand_index % bg_spv_.size()];
	
	  for(int j=0;j<spv_length;j++)
	  	top_spv[item_id*spv_length+j] = spv[j];
      ...
      pair<std::string, vector<int> > image =
          image_database_[window[MyWindowDataLayer<Dtype>::IMAGE_INDEX]];

      cv::Mat cv_img;
      if (this->cache_images_) {
        pair<std::string, Datum> image_cached =
          image_database_cache_[window[MyWindowDataLayer<Dtype>::IMAGE_INDEX]];
      ...
      int x1 = window[MyWindowDataLayer<Dtype>::X1];
      int y1 = window[MyWindowDataLayer<Dtype>::Y1];
      int x2 = window[MyWindowDataLayer<Dtype>::X2];
      int y2 = window[MyWindowDataLayer<Dtype>::Y2];
      ...
      // get window label
      top_label[item_id] = window[MyWindowDataLayer<Dtype>::LABEL];
	    
	  // get window weight
      top_weight[item_id] = window[MyWindowDataLayer<Dtype>::WEIGHT];
  ...
  // added by wsf at 2016.3.7
  //LOG(INFO) << "top_weight[0]: " << top_weight[0]; 
  //LOG(INFO) << " prefetch_weight_: " << prefetch_weight_.cpu_data()[0];
  

}

template <typename Dtype>
void MyWindowDataLayer<Dtype>::LayerSetUp(
  ...
  // LOG(INFO) << " MY WINDOW DATA LAYER SET UP";
...
template <typename Dtype>
void MyWindowDataLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  //
  // CHECK( top[2]->cpu_data()[0]!=0 ) << "weight equals to 0";
  // CHECK( this->prefetch_weight_.cpu_data()[0]!=0 ) << "weight_ equals to 0";
  
  // LOG(INFO) << "top[2]: "<< top[2]->cpu_data()[0] << "prefetch_weight_: " << this->prefetch_weight_.cpu_data()[0] << " top[0]: " << top[0]->cpu_data()[0];
...
INSTANTIATE_CLASS(MyWindowDataLayer);
REGISTER_LAYER_CLASS(MyWindowData);
